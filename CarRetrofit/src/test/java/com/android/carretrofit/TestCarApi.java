...
public interface TestCarApi {
    ...
    Interceptor INTERCEPTOR = (command, parameters) -> {
        System.out.println("process command:" + command + " parameters:" + Arrays.toString(parameters));
        Thread.dumpStack();
...
//    void set6789ComboValues(@MultiParam ExampleUnitTest.CarData cardata);
    ...
    @Set(key = 6, value = @Value(Int = 10086))
    void set6IntValue();

    @Set(key = 9, value = @Value(stringArray = {"build", "in", "set"}))
