...
@SupportedAnnotationTypes({CartrofitProcessor.PROCESS_SUPER, CartrofitProcessor.SCOPE_IDENTIFIER})
...
public class CartrofitProcessor extends AbstractProcessor {
    ...
    static final String SCOPE_IDENTIFIER = PACKAGE_NAME + ".annotation.Scope";
    static final String PUBLISH_ATTR = "publish";
            ...
            if (kind == ElementKind.CLASS || kind == ElementKind.INTERFACE) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (kind == ElementKind.CLASS && PROCESS_SUPER.equals(annotationName)) {
                        ...
                        break;
                    } else if (kind == ElementKind.INTERFACE && SCOPE_IDENTIFIER.equals(annotationName)) {
                        logI("process Car Api element:" + element);
                        processCarApiClass(list.get(i), (TypeElement) element);
                        break;
                    }
                }
            }
    ...
    private void processCarApiClass(AnnotationMirror scopeAnnotation, TypeElement element) {
        for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> entry
                : scopeAnnotation.getElementValues().entrySet()) {
            ExecutableElement method = entry.getKey();
            AnnotationValue value = entry.getValue();
            String methodName = method.getSimpleName().toString();
            if (PUBLISH_ATTR.equals(methodName)) {
                boolean isPublish = (boolean) value.getValue();
                if (!isPublish) {
                    return;
                }
            }
